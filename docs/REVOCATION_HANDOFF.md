...
**Status:** Phases 0–4 DONE + verified + pushed. **Phase 5 (epoch-faithful durability reconciler + tombstones) is the immediate next step.**
...
| **5** | **Durability reconciler (epoch-faithful) + tombstones** | ⏭️ **NEXT (not started)** | — |
| 6 | Docs honesty pass (SECURITY/THREAT_MODEL/PAIRING) + fold in GATE findings | ⬜ pending | — |

**Green baseline (sandbox OFF, Node 22):** unit 44/44 · integration 23/23 (×2) · e2e 3/3 · security 25/25 · mobile 4/4 · lint clean.
**Current HEAD:** `16d5b57` (P4) → `772573c` (handoff) → `aa6a72f` (P3) → `07f0fe9` (handoff doc) → `e136187` (lint) → `5b7ebf5` (P2) → `f2df0c2` (build-box Flatpak spike) → `d9076e6` (P1) → `02d154a` (P0).
...
## 6. NEXT — Phase 5 agent brief (see §3 workflow + the subagent-write gotcha in §9)
> Implement PHASE 5 (durability reconciler made EPOCH-FAITHFUL + durable cross-device tombstones — RT-FIX B4, B9). Phases 0–4 are committed. Two known holes remain in the durability layer: (B4) `_reconcileDurability` re-appends an orphaned row via `_makeOp`, which re-seals under the ACTIVE epoch — pulling pre-rotation content forward into the new epoch and (pre-Phase-0 keyId fix) manufacturing known-plaintext correlates; the fix is re-appending under the row's ORIGINAL `epochTag`. (B9) delete-supersede is device-local, so a remote device's reconciler can RESURRECT a row another device deleted; the fix is the durable `tombstone!` family honored by both the reconciler and the reducer.
>
> READ FIRST: `docs/REVOCATION_DESIGN.md` §3.9 + §5.12 + Phase 5 plan (§8). Then `backend/autobase-sync.js` (`_reconcileDurability`, `_pendingDurable` map + where entries are recorded, `_makeOp` — NOTE the Phase 4 lifecycle-epoch-0 branch; the pending entry must carry+thread its ORIGINAL `epochTag` per §3.9, design hook comment at `_makeOp` ~:218), `backend/materialized-view.js` (`tombstone!` skeleton from Phase 1 — `PREFIX.TOMBSTONE`/`putTombstone`/`isTombstoned` may be partial; `_rowPresentFor` in autobase-sync), `_applyNoteDelete`/`_applyClipDelete` (hard vs soft delete), handoff §2 Phase 3/4 outcome notes.
>
> IMPLEMENT: (1) EPOCH-FAITHFUL RE-APPEND — `_pendingDurable` entries record the `epochTag` the op was originally sealed under; `_makeOp` (or a re-append variant) accepts an explicit `epochTag` override and seals under THAT key (survivors hold every key; pulling forward gains nothing and leaks linkage). Lifecycle ops stay epoch-0 (Phase 4). (2) DURABLE TOMBSTONES — deletes write a `tombstone!` row (keyed by objectBlindId, with the delete's lamport); `_rowPresentFor` treats tombstoned objects as present (retires the pending entry); the reducer checks `isTombstoned` before applying any `*_UPSERT` whose lamport does not beat the tombstone (no resurrection of cross-device deletes; LWW still allows a genuinely NEWER upsert). (3) Suppress re-append for objects whose creation epoch ≤ a just-revoked epoch during the fresh-writer migration window (§3.9 step 4, belt-and-suspenders).
>
> TESTS (deterministic `@hyperswarm/testnet`, template `test/integration/revocation-rotation.test.js`): (a) force a fresh-writer migration rollback after a rotation — the reconciler re-appends the orphaned row under its ORIGINAL epoch (assert the re-appended envelope's `header.epochTag`/keyId equal the original, NOT the active); (b) cross-device delete is NOT resurrected by a remote reconciler; (c) a re-appended row's keyId is NOT linkable across epochs (keyIdFor differs); (d) reorg truncating the KEY_ROTATE tail recomputes identical epoch state (no wedge, B11 regression).
>
> VERIFY (sandbox OFF, Node 22): FULL suite green + lint; integration 2×. Don't regress the decisive test, SB1, the firewall tests, or the B1 pairing tests.
>
> CONSTRAINTS: edit `backend/autobase-sync.js`, `backend/materialized-view.js`. Do NOT touch crypto-envelope.js; shared-ops.js only if a tombstone op type is genuinely unavoidable (prefer deriving tombstones from existing *_DELETE ops). Do NOT implement Phase 6. Do NOT git commit. RETURN: per-change summary with file:line, where the original epochTag is captured and threaded, the tombstone read/write points, FULL per-suite tallies (integration 2×), deviations.
