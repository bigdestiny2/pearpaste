...
QRCode.prototype.save = function(file, callback) {
  ...
  // Vendored as ESM for the pear-electron renderer: no Node `fs` available,
  // and the renderer never writes files anyway. The .save() helper degrades
  // to a clear error; the .svg()/in-memory path used by qrToSvg is unaffected.
  callback(new Error('QRCode.save not supported in this build'));
};
